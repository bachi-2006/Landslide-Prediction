...
# Designated Regional Emergency Shelters with relief amenities and high-ground bedrock safety
EMERGENCY_SHELTERS = [
    {
        "id": "sh-shillong-1",
...
async def get_emergency_shelters(lat: Optional[float] = None, lon: Optional[float] = None):
    ...
    if lat is not None and lon is not None:
        for s in shelters:
            ...
            s["est_travel_time_min"] = max(5, round((dist_km / 28.0) * 60)) # ~28km/h hilly terrain speed
